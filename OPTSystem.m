classdef (Abstract) OPTSystem < handle 
    ...
    properties (Access = private)
        ...
        motorAxisXDisplacement = 0; %X - displacement of the motor axis from the centre of image volume in pixels (unbinned raw value)
        motorAxisZDisplacement = 0; %Z - displacement of the motor axis from the centre of image volume in pixels (unbinned raw value)
    ...
    methods
        ...
        function simulateProjections(obj,objective,pointObject,showBoolean)
            ...
            xAoRDisplacement = obj.getMotorAxisXDisplacement.*obj.getPixelSize/objective.getMagnification; % object space real x aor displacement in mm
            zAoRDisplacement = obj.getMotorAxisZDisplacement.*obj.getPixelSize/objective.getMagnification; % object space real z aor displacement in mm
            ...
            for i=1:length(theta)
                ...
                if length(pointObject)==1
                    [objectX,objectZ] = pointObject.getRotatedXZ(t,[xAoRDisplacement,zAoRDisplacement]); %object space [x,z] in mm loc after rotation
                    objectY = pointObject.getY;
                    I = obj.getPSFimage(objectX,objectY,objectZ,objective,imageX,imageY,xApPlane,yApPlane,psfscX,psfscY,P,opCentre);
                else
                    for currentPointObject = pointObject
                        [objectX,objectZ] = currentPointObject.getRotatedXZ(t,[xAoRDisplacement,zAoRDisplacement]); %object space [x,z] in mm loc after rotation
                        objectY = currentPointObject.getY;
                        psf = obj.getPSFimage(objectX,objectY,objectZ,objective,imageX,imageY,xApPlane,yApPlane,psfscX,psfscY,P,opCentre);
                        I = I + psf;
                    end
                end
    ...
    methods (Access = protected)
        ...
        %           motorAxisXDisplacement, position of rotation axis
        %           - double aorVector, constant motorAxisXDisplacement
        %           - [], uses existing obj.motorAxisXDisplacement (default 0)
            ...
            switch obj.axisDirection
                ...
                case 'vert'
                    [ramp_kernel] = repmat(OPTSystem.ramp_flat(obj.getWidth).',obj.getHeight,1);
                     ...
                     figure; imagesc(ramp_kernel); figure;
